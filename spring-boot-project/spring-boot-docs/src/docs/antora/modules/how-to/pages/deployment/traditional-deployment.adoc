...
Spring Boot 支持传统部署方式，也支持更现代的部署方式。
本节解答了关于传统部署的常见问题。
...
If your existing application has more than one javadoc:org.springframework.context.ApplicationContext[] (for example, if it uses javadoc:org.springframework.web.servlet.support.AbstractDispatcherServletInitializer[]) then you might be able to combine all your context sources into a javadoc:org.springframework.boot.SpringApplication[].
