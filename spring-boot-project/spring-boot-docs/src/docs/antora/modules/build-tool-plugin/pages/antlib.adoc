...
要使用该模块，您需要在 `build.xml` 中声明额外的 `spring-boot` 命名空间，如以下示例所示：
...
TIP: “使用 Spring Boot”部分包含一个更完整的示例，参见 xref:reference:using/build-systems.adoc#using.build-systems.ant[使用 Apache Ant 和 `spring-boot-antlib`]。
